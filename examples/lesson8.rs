// Active ブザー を鳴らす。（1秒ごとにON/OFF切り替え ）
...
use hal::{
    gpio::{Output, PinState, PushPull, PB1},
    pac,
    prelude::*,
    timer::{CounterUs, Event},
};

static PB1: Mutex<RefCell<Option<PB1<Output<PushPull>>>>> = Mutex::new(RefCell::new(None));
...
fn TIM2() {
    ...
    cortex_m::interrupt::free(|cs| {
        ...
        let mut pb1 = PB1.borrow(cs).borrow_mut();
        let pb1 = pb1.as_mut();
        let pb1 = if let Some(pb1) = pb1 {
            pb1
        } else {
